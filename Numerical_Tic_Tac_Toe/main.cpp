...
protected:
    string name;
    T symbol;
    BoardGame_Classes<T>* boardPtr;  // Pointer to the board
public:
    ...
    T getsymbol();
    string getname();
    void setBoard(BoardGame_Classes<T>* b);
};
...
class GameManager {
private:
    BoardGame_Classes<T>* boardPtr;
    Player<T>* players[2];
public:
    GameManager(BoardGame_Classes<T>*, Player<T>* playerPtr[2]);
...
template <typename T>
GameManager<T>::GameManager(BoardGame_Classes<T>* bPtr, Player<T>* playerPtr[2]) {
    boardPtr = bPtr;
...
template <typename T>
void Player<T>::setBoard(BoardGame_Classes<T>* b) {
    this->boardPtr = b;
}
